...
| shop-service | 50056 | ✅ | ✅ | ✅ (Market) |
| payment-service | 50057 | ✅ | ✅ | ✅ (Market) |
| ai-inference-service | 50058 | ✅ | ✅ | ❌ |
| cartridge-service | 50059 | ✅ | ✅ | ❌ |
| calibration-service | 50060 | ✅ | ✅ | ❌ |
| coaching-service | 50061 | ✅ | ✅ | ❌ |
| notification-service | 50062 | ✅ | ✅ | ❌ |
| family-service | 50063 | ✅ | ✅ | ❌ |
| health-record-service | 50064 | ✅ | ✅ | ❌ |
| telemedicine-service | 50065 | ✅ | ✅ | ❌ |
| reservation-service | 50066 | ✅ | ✅ | ❌ |
| community-service | 50067 | ✅ | ✅ | ✅ |
| admin-service | 50068 | ✅ | ✅ | ❌ |
| prescription-service | 50069 | ✅ | ✅ | ✅ (Market) |
| translation-service | 50070 | ✅ | ✅ | ❌ |
| video-service | 50071 | ✅ | ✅ | ❌ |
| vision-service | — | ✅ | ✅ | ❌ |
| gateway | :8080 | ✅ | — | (자체) |

**요약**: 23/23 빌드 성공, 22/22 테스트 보유, **5/21 Gateway REST 라우트 등록** (치명적 갭)
...
### 5.1 치명적 (Critical) — 즉시 수정 필요

#### C-1: Gateway REST 라우트 미등록 (16/21 서비스)

**현상**: `rest_handler.go:SetupRoutes()`에 5개 라우트 그룹만 등록

```go
// 현재 등록된 라우트 (rest_handler.go:81-98)
h.registerAuthRoutes(mux)         // ✅
h.registerUserRoutes(mux)         // ✅
h.registerMeasurementRoutes(mux)  // ✅
h.registerMarketRoutes(mux)       // ✅ (Shop+Payment+Prescription)
h.registerCommunityRoutes(mux)    // ✅
```

**미등록 서비스 (16개)**:

| 서비스 | Flutter REST Client 엔드포인트 | Gateway 라우트 파일 |
|--------|-------------------------------|-------------------|
| device | `GET /api/v1/devices/*` | ❌ 없음 |
| subscription | `GET /api/v1/subscriptions/*` | ❌ 없음 |
| ai-inference | `POST /api/v1/ai/*` | ❌ 없음 |
| cartridge | `GET /api/v1/cartridges/*` | ❌ 없음 |
| calibration | `POST /api/v1/calibration/*` | ❌ 없음 |
| coaching | `GET /api/v1/coaching/*` | ❌ 없음 |
| notification | `GET /api/v1/notifications/*` | ❌ 없음 |
| family | `GET /api/v1/families/*` | ❌ 없음 |
| health-record | `GET /api/v1/health-records/*` | ❌ 없음 |
| telemedicine | `POST /api/v1/telemedicine/*` | ❌ 없음 |
| reservation | `POST /api/v1/reservations/*` | ❌ 없음 |
| admin | `GET /api/v1/admin/*` | ❌ 없음 |
| translation | `POST /api/v1/translations/*` | ❌ 없음 |
| video | `POST /api/v1/video/*` | ❌ 없음 |
| vision | — | ❌ 없음 |

**영향**: Flutter REST Client가 이 16개 서비스 엔드포인트를 호출하면 **404 Not Found** 반환.
단, Flutter는 gRPC 직접 연결 + 시뮬레이션 폴백을 사용하므로 개발/테스트 환경에서는 동작하지만, **프로덕션 REST-only 배포 시 치명적**.

**수정 방법**: 각 서비스별 `register{Service}Routes(mux)` 함수 작성 및 `SetupRoutes()`에 등록
...
| Gateway REST 라우트 | 21개 서비스 | 5개 등록 | **23.8%** ⚠️ |
...
#### 최우선 (Week 1)
1. **Gateway REST 라우트 16개 서비스 등록** — Flutter REST Client가 정상 동작하려면 필수
2. **상품 리뷰 API 실구현** — 현재 스텁 상태

#### 높은 우선순위 (Week 2)
3. **DataHubScreen API 바인딩 완성**
4. **GuardianDashboardScreen API 연결**
5. **PG 결제 실연동** (토스페이먼츠/NHN KCP)
6. **FCM 푸시 알림 연동**

#### 중간 우선순위 (Week 3~4)
7. **WebRTC 시그널링 서버 구축** + VideoCallScreen 연동
8. **본인인증 PASS/KCB 연동**
9. **AI 스트리밍 SSE 실연동**
10. **약국 인터페이스 외부 시스템 연동**
...
**개선 필요**:
- Gateway가 **병목**: 21개 gRPC 서비스 중 5개만 REST 노출
- 외부 연동 (PG, 인증, FCM, 약국)이 모두 프레임만 존재
