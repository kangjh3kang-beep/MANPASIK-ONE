...
| MPS-FMEA-001 | FMEA (고장 모드 영향 분석) | ✅ v1.0 완료 (38개 고장 모드, 8개 서브시스템) |
| MPS-FTA-001 | FTA (결함 트리 분석) | 작성 예정 (Phase 2) |
| MPS-PHA-001 | 사전 위해 분석 (PHA) | ✅ v1.0 완료 (34개 위해, 7개 카테고리) |
...
Phase 1 (MVP, Month 1-4):
  ├── [완료] 사전 위해 분석 (PHA) — 34개 위해, 7개 카테고리
  ├── [완료] 위험관리 계획서 (RMP) v1.0
  ├── [완료] FMEA v1.0 — 38개 고장 모드, 8개 서브시스템
  └── [예정] 위험 분석 보고서 초안
...
| 1.1 | 2026-02-14 | FMEA/PHA 완료 반영, 타임라인 업데이트 | Claude |
