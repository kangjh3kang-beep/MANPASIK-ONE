...
echo ""
echo "=== B2 대상 서비스 테스트 ==="
B2_SERVICES=(iot-gateway-service concept-service voice-profile-service data-platform-service)
T_PASS=0
T_FAIL=0
for svc in "${B2_SERVICES[@]}"; do
  DIR="$BASE/$svc"
  cd "$DIR"
...
echo "=== B2 테스트: $T_PASS PASS / $T_FAIL FAIL ==="
