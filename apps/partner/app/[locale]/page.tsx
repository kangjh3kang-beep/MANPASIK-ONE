...
import { Users2, Building2, ArrowLeftRight, CheckCircle2, Clock, AlertCircle, Link2, Globe } from 'lucide-react';
...
export default function PartnerPage() {
  ...
  return (
    <>
      <DomainNav currentDomain="partner" />
      ...
      <div className="border-b border-slate-200 bg-white px-8 py-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-slate-900">파트너 통합 연동</h1>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-8 py-8 space-y-8">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <div className="space-y-3">
            {PARTNERS.map(p => {
              const st = STATUS_STYLE[p.status as keyof typeof STATUS_STYLE];
              return (
                <div key={p.name} className="flex items-center justify-between rounded-xl border border-slate-100 px-5 py-4 hover:shadow-md transition-shadow">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-bold text-slate-700">{p.exchanges.toLocaleString()}건</p>
                    <p className="text-[11px] text-slate-400 flex items-center gap-1 justify-end"><Clock className="h-3 w-3" /> {p.lastSync}</p>
                  </div>
                </div>
              );
            })}
